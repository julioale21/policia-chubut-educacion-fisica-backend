...
export class ExerciseCompletionsService {
  ...
  async create(
    ...
    try {
      const { routineExerciseId, routineAssignmentId, ...completionData } =
        createExerciseCompletionDto;
      ...
      if (!routineExercise) {
        throw new NotFoundException(
          `RoutineExercise with ID "${routineExerciseId}" not found`,
        );
      }
  ...
  async findAll() {
    return await this.exerciseCompletionRepository.find({
      relations: ['routineExercise', 'routineAssignment'],
    });
  }
  ...
  async update(
    ...
    try {
      ...
      const { routineExerciseId, routineAssignmentId, ...updateData } =
        updateExerciseCompletionDto;
      ...
      if (routineExerciseId) {
        ...
        exerciseCompletion.routineExercise = routineExercise;
      }
